...
function HitungText(){
	var teks = $("#sendie").val();
	var jumlahtext = teks.length;

	console.log(jumlahtext);

	if( jumlahtext > 1){

		var dataString = "chatid=" + chatid + "&digitText=" + jumlahtext;
		$.ajax({
			type: "POST",
			url: "https://www.dfunstation.com/api4/consultant/index.php/member/tesmengetik/?callback=?",
			data: dataString,
			crossDomain: true,
			cache: false,
			success: function(data) 
			{ 
				if(data['status'] == "OK" ){
					console.log("Berhasil Mengetik");
				}
			}
		});

	} else if( jumlahtext < 1 ) {
...
function updateChatx() {
	...
	if (!instanse) {
		...
		$.ajax({
			...
			success: function (data) {
				if (data.text) {
					$("#statusheader").html(data['is_ketik']);
